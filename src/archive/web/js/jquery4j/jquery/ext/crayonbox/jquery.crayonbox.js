...
jQuery.fn.crayonbox = function(opt)
{
	...
	return this.each(function()
	{
		...
		$(self).click(function(e)
		{
			...
			// optional callback
			options.onSelection.call(self,$(clicked).attr('title'));
		});
		...
		{
			//zk.log(i + typeof(options.colors[i]));
			//following is modified, because zk enhanced Array in js.js
			if (typeof(options.colors[i]) != "function") {
				$('<' + options.crayonTag + ' class="crayon" title="' + options.colors[i] + '">&nbsp;</' + options.crayonTag + '>').appendTo($(self)).css('background-color', options.colors[i]);
			}
		} 
